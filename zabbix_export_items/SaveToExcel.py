...
def WriteExcel(FilaPath, ZabbixData):
    ...
    TableTitle = ['IP','主机名','运行时长/天','CPU/核','内存/GB','根目录总量/G','根目录使用量/G','根目录使用率/%','CPU平均负载/1min','CPU平均负载/5min','CPU平均负载/15min','CPU空闲时间','CPU使用率/%','内存使用率/%','可用内存/G','磁盘使用率/%']
    ...
    for host in ZabbixData.values():
        ...
        HostItemValues.append(str(round(int(host['System uptime']) / 24 / 60 / 60, 2)) + 'd')  # 首先将运行时长换算为天数，然后再加入到列表中
        ...
        HostItemValues.append(str(round(float(host['/: Space utilization']), 2)) + '%')  # 根目录使用率
        ...
        HostItemValues.append(round(float(host['CPU idle time']), 2))  # CPU空闲时间
        ...
        HostItemValues.append(host['Disk utilization'])  # 磁盘使用率
    ...
    # 设置列宽
    column_width = {'A': 18, 'B': 30, 'C': 14, 'D': 10, 'E': 10, 'F': 16, 'G': 18, 'H': 18, 'I': 22, 'J': 22, 'K': 23,
